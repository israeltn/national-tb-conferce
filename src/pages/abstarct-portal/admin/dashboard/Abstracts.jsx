...
//    ];
 ...
 useEffect(() => {
  axios.get('/sanctum/csrf-cookie').then(response => {
      axios.get(`/api/view-abstracts`).then(res=>{
        if(res.status === 200) 
        {
        ...
        return (
          <tr key={i}>
            ...
            <td className="justify-center items-center text-center px-6 py-4 whitespace-nowrap text-sm font-medium">
                         <Link to={`http://localhost:9999/${item.image}`} className="text-indigo-600 px-2 hover:text-indigo-900 ">
                         ...
                         {/* <Link to="#" className="text-indigo-600 px-2 hover:text-indigo-900 ">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className=" stroke-red-700 w-4 h-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                          </svg>
       ...
       <div className="flex mx-3 mt-2 flex-col">
          <div className="md:flex items-center justify-between mx-4 my-2 sm:mt-2">
                    <div className='flex md:justify-start md:items-start text-center'>
                         <h2 className="text-gray-600 mt-2 lg:mt-8 md:text-xl text-sm font-semibold text-center">Abstracts</h2>
                    </div>
