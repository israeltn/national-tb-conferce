...
export const AddThematicArea = () => {
  ...
  const registerSubmit = async (e) => {
    ...
    const data = {
        ...
        discription: registerInput.discription,
    ...
    axios.get("/sanctum/csrf-cookie").then((response) => {
      ...
      axios.post(`/api/reg-user`, data).then((res) => {
        if (res.data.status === 200) {
          ...
          navigate("/dashboard/addthematic");
        } else {
          setRegister({
            ...registerInput,
              ...
              <div className="px-5 py-7 justify-center item-center">
                <div className="mb-5">
                  ...
                  <span className="pb-2 mb-2 text-xs text-red-600">
                    {registerInput.error_list.title}
                  </span>
                </div>
                <div className="mb-5">
                  <label className="font-semibold text-sm text-gray-600  block">
                  ...
                  <input
                    ...
                    name="discription"
                    onChange={handleInput}
                    value={registerInput.discription}
                    className="border rounded-lg px-3 py-2 mt-1 text-sm w-full"
                  />
                  <span className="pb-2 mb-2 text-xs text-red-600">
                    {registerInput.error_list && registerInput.error_list.discription}
                  </span>
                </div>
                ...
                <div className="item-center justify-center flex">
                  ...
                  >
                    <span className="inline-block mr-2">
                      {" "}
                      {isSubmitting ? "Submitting Data..." : "Create"}
                    </span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
