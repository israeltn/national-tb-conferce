...
export const UpdateUser = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios.get(`/api/single_user/${id}`).then((response) => {
      // console.log(response.data.user);
      const userData = response.data.user; // Assuming response.data contains user data
      setUpdateInput(userData);
      setLoading(false);
    }).catch((error) => {
              ...
              <div className="px-5 py-7 justify-center item-center">
                <div className="mb-5">
                  ...
                  <input
                    ...
                    placeholder={updateInput.firstname}
                    className="border rounded-lg px-3 py-2 mt-1 text-sm w-full"
                  />
                  <span className="pb-2 mb-2 text-xs text-red-600">
                    {updateInput.error_list && updateInput.error_list.firstname}
                  </span>
                </div>
                <div className="mb-5">
                  <label className="font-semibold text-sm text-gray-600  block">
                  ...
                  />
                  <span className="pb-2 mb-2 text-xs text-red-600">
                    {updateInput.error_list && updateInput.error_list.lastname}
                  </span>
                </div>
                <div className="mb-5">
                  <label className="font-semibold text-sm text-gray-600 block">
                  ...
                  />
                  <span className="pb-2 mb-2 text-xs text-red-600">
                    {updateInput.error_list && updateInput.error_list.email}
                  </span>
                </div>
                <div className="mb-5">
                  <label className="font-semibold text-sm text-gray-600 block">
                  ...
                  <span className="pb-2 mb-2 text-xs text-red-600">
                    {updateInput.error_list && updateInput.error_list.role}
                  </span>
                </div>
                <div className="mt-5 mb-5">
                  ...
                  />
                  <span className="pb-2 mb-2 text-xs text-red-600">
                    {updateInput.error_list && updateInput.error_list.password}
                  </span>
                </div>
                <div className="item-center justify-center flex">
                  ...
                  >
                    <span className="inline-block mr-2">
                      {" "}
                      {isSubmitting ? "Submitting Data..." : "Update"}
                    </span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
