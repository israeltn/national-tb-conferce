...
export const BoardApproveViewAbstract = (props) => {
  const url= baseURL;
...
//   const [errorlist, setError] = useState([]);
  ...
  const handleInput = (e) => {
    e.persist();
    setAbstractData({ ...abstractData, [e.target.name]: e.target.value });
  };
  ...
  const updateAbstract = (e) => {
    ...
    setLoading(true);
     const data = {
          ...
          remark: abstractData.remark,
     } 
  axios.get('/sanctum/csrf-cookie').then(response => {
    axios.post(`/api/update-abstract-aprover/${id}`, data).then(res =>{ 
         if(res.data.status === 200) {
        
         swal("Success",res.data.message, "success");
             setLoading(false);   
             navigate("/boarddashboard/abstracts");
         }
         else if(res.data.status === 404)                 
         {
         swal("Error",res.data.message, "error"); 
         ...
         else if(res.data.status === 500)
         {
          ...
          navigate("/boarddashboard/abstracts"); 
              ...
              setAbstract({...abstractData, error_list: res.data.validation_errors});
         }
         setLoading(false);  
...
<div className="full bg-slate-300 h-[0.1rem]"></div>
        <div className="flex flex-wrap -mx-3 mt-2 mb-4">
          <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            ...
            <p className=" w-full uppercase text-xs  text-gray-900  focus:ring-blue-500 focus:border-blue-500     dark:focus:ring-blue-500 dark:focus:border-blue-500">
              {" "}
              {Singleabstract.status}{" "}
            </p>
          </div>
          ...
          <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            ...
            <p className=" w-full  text-xs  text-gray-900  focus:ring-blue-500 focus:border-blue-500     dark:focus:ring-blue-500 dark:focus:border-blue-500">
              {" "}
              {Singleabstract.remark}{" "}
            </p>
          </div>
        </div>
    ...
    <div className="items-center w-full justify-center  px-3 mb-6 md:mb-0">
        ...
        <div className="w-1/2 flex justify-center items-center text-center ">
            <select
                ...
                value={abstractData.status}
                className="block w-1/4 uppercase items-center justify-center font-bold text-xs appearance-none md:w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-gender"
            >
                {/* <option className="font-bold">{abstractInput.status}</option> */}
                <option className="font-bold">Select</option>
                <option className="text-green-500 font-bold" value="approved">
                Approved 
                </option>
                <option className="text-yellow-500 font-bold" value="approved-with-revision">
            ...
            <span className="pb-2 mb-2 text-sm text-red-600">
                {abstractInput.error_list && abstractData.error_list.status}
            </span>
        </div>
        </div>
        ...
        <div className="mt-4 w-full">
            ...
            <textarea
                ...
                name="remark"
                onChange={handleInput}
                value={abstractData.remark}
                placeholder="Enter Comments here..."
            ></textarea>
            <span className="pb-2 mb-2 text-sm text-red-600">
                {abstractData.error_list && abstractData.error_list.remark}
            </span>
        </div>
    </div>
  </div>
