...
//   const [errorlist, setError] = useState([]);
  ...
  useEffect(() => {
    axios.get(`/api/edit-asigned-abstract/${id}`).then((res) => {
      ...
      navigate("/reviewerdashboard/asignedabstract"); 
      }
      setLoading(false);
    });
  }, [id, navigate]);
  ...
  const updateAbstract = (e) => {
    ...
    setLoading(true);
     ...
     axios.get('/sanctum/csrf-cookie').then(response => {
          axios.post(`/api/update-abstract/${id}`, data).then(res =>{ 
               if(res.data.status === 200) {
    ...
    <div className="items-center justify-center md:w-1/4 px-3 mb-6 md:mb-0">
        ...
        <div className="relative">
            ...
            >
                ...
                <option className="text-green-800 font-bold" value="approved">
                Approved  
                </option>
                <option className="text-yellow-500 font-bold" value="approved-with-revision">
                Approved with revision 
                </option>
                <option className="text-red-700 font-bold" value="not-approved">
                    Not Approved
                </option>
